 ...
 * https://webmidijs.org
 * Build generated on April 21st, 2024.
 *
...
/* Version: 3.1.9 - April 21, 2024 09:53:24 */
(function (exports) {
  "use strict";
      ...
      if (event === undefined) {
        ...
        if (this.eventMap[event] && this.eventMap[event].length > 0) {
          if (callback instanceof Listener) {
            let result = this.eventMap[event].filter(
  ...
  class Listener {
    ...
    constructor(event, target, callback, options = {}) {
       ...
       */
      this.remaining =
        parseInt(options.remaining) >= 1
          ? parseInt(options.remaining)
          : Infinity;
  ...
  class Note {
    ...
    getOffsetNumber(octaveOffset = 0, semitoneOffset = 0) {
      ...
      return Math.min(
        Math.max(this.number + octaveOffset * 12 + semitoneOffset, 0),
        127
      );
    }
  }
  ...
  class Utilities {
    ...
    static buildNote(input, options = {}) {
      ...
      if (number === false) {
        ...
        throw new TypeError(
          `The input could not be parsed as a note (${input})`
        );
      }
    ...
    static offsetNumber(number, octaveOffset = 0, semitoneOffset = 0) {
      ...
      return Math.min(
        Math.max(number + octaveOffset * 12 + semitoneOffset, 0),
        127
      );
    }
    ...
    static getCcNumberByName(name) {
      let message = Enumerations.CONTROL_CHANGE_MESSAGES.find(
  ...
  class OutputChannel extends EventEmitter {
    ...
    sendKeyAftertouch(target, pressure, options = {}) {
      if (wm.validation) {
        ...
        if (options.rawValue) {
          if (
            !(pressure >= 0 && pressure <= 127 && Number.isInteger(pressure))
          ) {
      ...
      // Plot total offset
      const offset =
        wm.octaveOffset + this.output.octaveOffset + this.octaveOffset;
      ...
      for (let note of notes) {
        if (parseInt(note.duration) > 0) {
          const noteOffOptions = {
            time:
              (Utilities.toTimestamp(options.time) || wm.time) +
    ...
    sendNoteOff(note, options = {}) {
      if (wm.validation) {
        ...
        if (
          ...
          !(options.release >= 0 && options.release <= 1)
        ) {
          throw new RangeError(
    ...
    sendNoteOn(note, options = {}) {
      if (wm.validation) {
        if (
          options.rawAttack != undefined &&
          !(options.rawAttack >= 0 && options.rawAttack <= 127)
        ) {
          throw new RangeError(
        ...
        if (
          options.attack != undefined &&
          !(options.attack >= 0 && options.attack <= 1)
        ) {
          throw new RangeError(
    ...
    sendModulationRange(semitones, cents, options = {}) {
      ...
      if (wm.validation) {
        ...
        if (
          !(cents == undefined) &&
          (!Number.isInteger(cents) || !(cents >= 0 && cents <= 127))
        ) {
          throw new RangeError(
    ...
    sendNrpnValue(nrpn, data, options = {}) {
      ...
      if (wm.validation) {
        if (
          !Array.isArray(nrpn) ||
          !Number.isInteger(nrpn[0]) ||
          !Number.isInteger(nrpn[1])
        ) {
    ...
    sendTuningBank(value, options = {}) {
      if (wm.validation) {
        if (!Number.isInteger(value) || !(value >= 0 && value <= 127)) {
          throw new RangeError(
            "The tuning bank number must be between 0 and 127."
          );
        }
      }
    ...
    set octaveOffset(value) {
      if (this.validation) {
        ...
        if (isNaN(value))
          throw new TypeError(
            "The 'octaveOffset' property must be an integer."
          );
      }
    ...
    sendSysex(identification, data = [], options = {}) {
      ...
      if (data instanceof Uint8Array) {
        const merged = new Uint8Array(
          1 + identification.length + data.length + 1
        );
        ...
        const merged = identification.concat(
          data,
          Enumerations.SYSTEM_MESSAGES.sysexend
        );
    ...
    sendTimecodeQuarterFrame(value, options = {}) {
      if (wm.validation) {
        ...
        if (isNaN(value) || !(value >= 0 && value <= 127)) {
          throw new RangeError(
            "The value must be an integer between 0 and 127."
          );
        }
      }
  ...
  class InputChannel extends EventEmitter {
    ...
    _parseEventForStandardMessages(e) {
      ...
      if (
        ...
        namedEvent.type =
          `${event.type}-` + Enumerations.CONTROL_CHANGE_MESSAGES[data1].name;
    ...
    _dispatchParameterNumberEvent(type, paramMsb, paramLsb, e) {
      ...
      if (type === "rpn") {
        event.parameter = Object.keys(Enumerations.REGISTERED_PARAMETERS).find(
          (key) => {
            return (
              Enumerations.REGISTERED_PARAMETERS[key][0] === paramMsb &&
      ...
      // Type and subtype
      const subtype =
        Enumerations.CONTROL_CHANGE_MESSAGES[e.message.dataBytes[0]].name;
    ...
    addListener(event, listener, options = {}) {
      if (wm.validation) {
        ...
        if (typeof options === "function") {
          let channels =
            listener != undefined ? [].concat(listener) : undefined; // clone
          listener = options;
          options = {
            channels: channels,
          };
      ...
      if (Enumerations.CHANNEL_EVENTS.includes(event)) {
        ...
        Utilities.sanitizeChannels(options.channels).forEach((ch) => {
          listeners.push(
            this.channels[ch].addListener(event, listener, options)
          );
        });
        return listeners;
      } else {
  ...
  class WebMidi extends EventEmitter {
    ...
    getOctave(number) {
      ...
      if (!isNaN(number) && number >= 0 && number <= 127) {
        return Utilities.getNoteDetails(
          Utilities.offsetNumber(number, this.octaveOffset)
        ).octave;
      } else {
        return false;
      }
    ...
    async _updateInputs() {
      ...
      this.interface.inputs.forEach((nInput) => {
        ...
        if (!this._inputs.find((input) => input._midiInput === nInput)) {
          ...
          let input = this._disconnectedInputs.find(
            (input) => input._midiInput === nInput
          );
    ...
    async _updateOutputs() {
      ...
      this.interface.outputs.forEach((nOutput) => {
        ...
        if (!this._outputs.find((output) => output._midiOutput === nOutput)) {
          ...
          let output = this._disconnectedOutputs.find(
            (output) => output._midiOutput === nOutput
          );
